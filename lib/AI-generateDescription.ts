...
export const AI_generateDescription = async (title: string) => {
    const prompt = `generate a description for this task: ${title}`;
  try {
    const response = await axios.post(
      'https://openrouter.ai/api/v1/chat/completions',
      {
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a helpful AI task assistant. Provide a direct answer.',
          },
          {
            role: 'user',
            content: prompt,
